...
int keymap_editor_assign(struct keymap_editor_model *model,
  ...
  binding.keycode = keycode;
  binding.row = model->targets[target_index].row;
  binding.column = model->targets[target_index].column;
  binding.flags = model->targets[target_index].mapping_flags;
  if ((host_flags & KEYMAP_EDITOR_MAP_MOD_SHIFT) != 0 &&
      (model->targets[target_index].flags & KEYMAP_EDITOR_VIRTUAL_SHIFT) == 0 &&
      (binding.flags & (KEYMAP_EDITOR_LEFT_SHIFT |
                        KEYMAP_EDITOR_RIGHT_SHIFT |
                        KEYMAP_EDITOR_SHIFT_LOCK)) == 0) {
    binding.flags &= ~KEYMAP_EDITOR_ALLOW_SHIFT;
  ...
  binding.flags |= host_flags;
  model->bindings[model->binding_count++] = binding;
  return 1;
}
