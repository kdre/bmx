...
void main(void) {
  ...
  if (u_horizontal_jitter_enable > 0.5) {
    ...
    float wave = sin(source_row * frequency + phase) * 0.68;
    wave += sin(source_row * frequency * 2.7 + 1.7 + phase * 2.0) * 0.32;
  ...
  if (u_noise_enable > 0.5) {
    ...
    float luma_noise =
        (noise_hash(source_pixel, noise_frame) - 0.5) *
        noise_normalization;
    float row_noise =
        (noise_hash(vec2(source_pixel.y, 17.0), noise_frame + 109.0) - 0.5) *
        noise_normalization;
    float signal_gain =
        0.35 + 0.75 * sqrt(clamp(luminance(color), 0.0, 1.0));
    color += vec3((luma_noise + row_noise * 0.55) * signal_gain *
                  max(u_luminance_noise, 0.0));
    float chroma_i =
