...
vec3 sample_source(vec2 uv) {
  ...
  return texture2D(u_texture, clamp(uv, half_texel, vec2(1.0) - half_texel)).rgb;
}
...
void main(void) {
  ...
  if (u_composite_artifacts_enable > 0.5) {
    ...
    chroma = mix(chroma, bleed_chroma, clamp(u_composite_color_bleed, 0.0, 1.0));
  ...
  if (u_uneven_illumination_enable > 0.5) {
    ...
    field += cos((screen_point.x * -0.45 + screen_point.y * 0.85) *
                 (sy * 0.7) + 5.19) * 0.16;
    color *= 1.0 + field * clamp(u_uneven_illumination_strength, 0.0, 0.35);
  }
  ...
  if (u_noise_enable > 0.5) {
    ...
    float row_noise =
        ...
        noise_normalization;
    float signal_gain = 0.35 + 0.75 * sqrt(clamp(luminance(color), 0.0, 1.0));
