...
void main(void) {
  ...
  if (u_glass_reflection_enable > 0.5) {
    ...
    // The Pi4 reference applies one full RGB565 quantum across the complete
    // continuous reflection field. Scaling the dither amplitude by the local
    // reflection reintroduces visible bands at the soft outer transition.
    continuous_modulation = max(continuous_modulation, 0.15);
  }
  if (continuous_modulation > 0.0) {
